...
export class GitManager {
  ...
  async createPullRequest(
    ...
    const diffResult = this.execRaw(["git", "diff", "--staged", "--quiet"], workspacePath);
    if (diffResult.status !== 0) {
      this.exec(["git", "commit", "-m", `feat: ${run.ticket.title} [AgentSDLC run ${run.run_id}]`], workspacePath);
    }
  ...
  private buildPRBody(run: TaskRun): string {
    ...
    // --- Footer ---
    lines.push(`---`);
    lines.push(`Generated by [AgentSDLC](https://github.com/Sagart-cactus/SprintFoundry)`);
